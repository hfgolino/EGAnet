...
simEGM.net <- function(
    ...
    sample.size, max.iterations = 1000, p.in, p.out
)
{
  ...
  nv = rep(variables[1], communities[1])
  p.in = rep(p.in, communities[1]) # probability of a node to be randomly linked to other nodes in the same community
  p.out = rep(p.out, communities[1]) # probability of a node to be randomly linked to nodes in other communities
  p.del = 0 # proportion of links that are randomly deleted
  net.simu <- wfg::network.simu(nv=nv, p.in=p.in, p.out=p.out, p.del=p.del)
  net <- net.simu$net
  group <- net.simu$group
  
  net.matrix <- as.matrix(igraph::as_adjacency_matrix(net))
  
  # Net structure times cor
  net.matrix.weights <- net.matrix*R
  net.matrix.weights < Matrix::forceSymmetric(net.matrix.weights)
  net.matrix.weights <- as.matrix(Matrix::nearPD(net.matrix.weights)$mat)
  diag(net.matrix.weights) <- 1

  
  # Obtain zero-order correlations from loadings
  R <- net.matrix.weights
  
  # Obtain partial correlations from loadings
  P <- silent_call(cor2pcor(net.matrix.weights))
