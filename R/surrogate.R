...
# Computing the Median, SE and CI of the Median:
  ...
  distributions.gather <- gather(distributions, Dist)
  ...
  if(plot.surrogate == TRUE) {
    g <- ggplot(summary.distribution, aes(x=Dist, y=Median, group=1)) +
      geom_point(aes(size=1000), alpha=0.52) +
      geom_errorbar(width=.1, aes(ymin=Lower, ymax=Upper), colour="darkred") +
      labs(x="Type",y= "Number of Dimensions", title="Mean and Confidence Intervals of the Number of Dimensions Assessed via EGA", subtitle="Observed (Random Subsample), Shuffling (Surrogate EGA) and Normal Distribution \nError Bar Using median as center with 95% Confidence Intervals", caption="Hudson Golino") +
