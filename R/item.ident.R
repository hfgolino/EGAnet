...
#' for each community. Based on \code{\link{item.confirm}}, researchers can flag
...
#' to first run \code{\link{item.confirm}} to determine appropriate cut-off
#' value.
#' Defaults to .80
#' 
...
#' 
#' item.confirm(boot.wmt, confirm = ega.wmt$wc, plot.ic = TRUE)
#' 
#' item.ident(boot.wmt, confirm = ega.wmt$wc, rep.val = .80)
#' }
#' 
#' @author Hudson F. Golino <hfg9s at virginia.edu> and Alexander Christensen <alexpaulchristensen@gmail.com>
#' 
#' @export
item.ident <- function (bootega.obj, confirm, rep.val = .80)
{
    ...
    net <- bootega.obj$EGA$network
        ...
        item.con <- item.confirm(bootega.obj,confirm,plot.ic=FALSE)
        ...
        #Standaridize
        std.item.id <- std.item.id[order(confirm),]
        ...
        result$unstd <- unstd.item.ident
        result$std <- std.item.ident
