...
int maximum_membership(int* memberships, int cols){
...
struct signed_louvain_result signed_louvain(
  ...
  // Compute next levels
  while(max_communities != (original_cols - 1)) {
...
SEXP r_signed_louvain(SEXP r_input_network, SEXP r_resolution, SEXP r_seed) {
